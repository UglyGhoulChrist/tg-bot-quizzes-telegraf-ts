...
export function changeListAnswersAndSelectFact(quizzes: IQuiz[]): IQuiz[] {
    return quizzes.map(({ options, correct, facts, ...quiz }) => {
        ...
        const optionsCopy: string[] = options.map(option => option.charAt(0).toUpperCase() + option.slice(1));
        ...
        const newOptions: string[] = [correctAnswer, ...randomOptions].sort(() => Math.random() - 0.5);
        ...
        const randomFact: string[] = facts ? [facts[Math.floor(Math.random() * facts.length)]] : [];
